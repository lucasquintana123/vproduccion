...
# =============================
st.set_page_config(
    page_title="Demo modelo exportación cacao",
    layout="wide"
)
...
Esta aplicación utiliza técnicas de inteligencia artificial y aprendizaje automático 
para analizar exportaciones de cacao y sus derivados, identificando qué operaciones 
tienen mayor probabilidad de ser consideradas exportaciones fiables.
...
Esta demo presenta únicamente una ejecución automática del sistema utilizando 
un conjunto de datos precargado.
""")
...
with st.expander("ℹ️ Información sobre la demo"):
    ...
    Este sistema utiliza Machine Learning para analizar exportaciones de cacao 
    y estimar qué operaciones comerciales presentan mayores probabilidades 
    de éxito y confiabilidad.
...
st.markdown("""
La matriz de confusión permite visualizar cuántas predicciones fueron correctas 
e incorrectas comparando los valores reales frente a los predichos por el modelo.
""")
...
st.markdown("""
Las siguientes visualizaciones muestran patrones encontrados por el modelo 
sobre exportaciones de chocolate hacia diferentes continentes.
""")

producto = "CHOCOLATE"

df_procesado["Categoria"] = df_procesado["Categoria"].astype(str).str.upper().str.strip()
...
if df_graf.empty:
    ...
    with col1:
        ...
        Esta gráfica muestra cuántas exportaciones fueron clasificadas como:
        - Fiables
        - No fiables
        """)
        ...
        fig1.update_traces(
            marker_color=[
                "#0D47A1" if str(c) == "0" else "#64B5F6"
                for c in conteo["Clase"]
            ],
    ...
    with col2:
        ...
        if fiables.empty:
            ...
            Se identifican los continentes donde existen mayores probabilidades 
            de éxito comercial según el modelo predictivo.
            """)

            conteo_cont = fiables["Continente_destino"].value_counts().reset_index()
            conteo_cont.columns = ["Continente", "Cantidad"]

            fig2 = px.bar(conteo_cont, x="Continente", y="Cantidad", text="Cantidad")
...
st.markdown("""
La inteligencia artificial genera automáticamente una interpretación 
de los resultados obtenidos por el modelo, facilitando la toma de decisiones.
""")
...
st.markdown("""
La tabla final contiene los datos procesados junto con la predicción 
generada automáticamente por el modelo.
""")

st.dataframe(df_procesado.head(20))
